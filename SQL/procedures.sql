...
BEGIN
  ...
  DECLARE vLAST_DATE_TIME INTEGER;

  SET cMAX_RECORDS = IFNULL(pMAX_RECORDS,1000);

  START TRANSACTION;
  SELECT CURRENT_INDEX INTO vDATA_INDEX FROM TRANSMITTER WHERE (TRANSMITTER_ID = pTRANSMITTER_ID) AND (PASSWORD_CODE = pPASSWORD_CODE) FOR UPDATE;
  ...
  END IF;
  SELECT DATA_TIME INTO vLAST_DATE_TIME FROM TRANSMITTER_DATA WHERE (TRANSMITTER_ID = pTRANSMITTER_ID) AND (PASSWORD_CODE = pPASSWORD_CODE) AND
                                                                    (DATA_INDEX = vDATA_INDEX);
  IF (pDATA_TIME - IFNULL(vLAST_DATE_TIME,0)) > 100 THEN
